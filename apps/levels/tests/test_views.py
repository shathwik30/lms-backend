...
class LevelPublicAPITests(APITestCase):
    def setUp(self):
        ...
        self.week = self.factory.create_week(self.level, order=1)
    ...
    def test_level_detail_includes_weeks(self):
        response = self.client.get(f"/api/v1/levels/{self.level.pk}/")
...
class AdminLevelAPITests(APITestCase):
    ...
    def test_admin_create_week(self):
        response = self.admin_client.post(
            f"/api/v1/levels/admin/{self.level.pk}/weeks/",
            {"name": "Week 1", "order": 1, "level": self.level.pk},
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
