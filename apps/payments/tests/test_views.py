...
class PurchaseModelTests(TestCase):
    ...
    def test_active_purchase_is_valid(self):
        purchase = self.factory.create_purchase(self.profile, self.data["course"])
        self.assertTrue(purchase.is_valid)

    def test_expired_purchase_is_not_valid(self):
        purchase = self.factory.create_expired_purchase(self.profile, self.data["course"])
...
class PaymentAPITests(APITestCase):
    ...
    def test_initiate_nonexistent_course_returns_404(self):
        response = self.client.post(
            "/api/v1/payments/initiate/",
            {
                "course_id": 99999,
    ...
    def test_unauthenticated_cannot_access_payments(self):
        anon = APIClient()
        response = anon.post("/api/v1/payments/initiate/", {"course_id": 1})
...
class PaymentAmountVerificationTests(APITestCase):
    """Tests for amount verification between transaction and course price."""
    ...
    def test_amount_mismatch_returns_400(self):
        """If txn.amount != course.price at verify time, should return 400."""
        ...
        txn.amount = self.data["course"].price + 100
    ...
    def test_amount_match_succeeds(self):
        """When txn.amount == course.price, verify should succeed normally."""
        init_resp = self.client.post(
            "/api/v1/payments/initiate/",
            {
                "course_id": self.data["course"].pk,
    ...
    def test_course_price_change_between_initiate_and_verify_is_caught(self):
        """If course price changes after initiate but before verify, mismatch is caught."""
        init_resp = self.client.post(
            "/api/v1/payments/initiate/",
            {
                "course_id": self.data["course"].pk,
        ...
        # Change the course price after initiation
...
class PaymentEdgeCaseTests(APITestCase):
    ...
    def test_inactive_course_returns_404_on_initiate(self):
        """Initiating payment for an inactive course should return 404."""
        course = self.data["course"]
